...
if_statement: IF expression THEN {prepare_if_stmt($2);} statement if_tail {$$ = statement_if($2,$5,$6);}
;
...
while_statement: WHILE expression {prepare_while_stmt($2);} DO statement {$$ = statement_while($2,$5);}
;

for_statement: FOR ID ASSIGN iter_space DO {$<var>$ = prepare_for_stmt($2);} statement {$$ = statement_for($<var>6,$4,$7);}
;
