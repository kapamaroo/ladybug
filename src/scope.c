...
void start_new_scope(func_t *scope_owner) {
    ...
    if (sm_scope<MAX_SCOPE) {
        scope_owner->symbol_table.pool = (sem_t**)calloc(MAX_SYMBOLS,sizeof(sem_t*));
        ...
        scope_owner->symbol_table.lost = (char**)calloc(MAX_LOST_SYMBOLS,sizeof(char*));
...
void start_new_with_statement_scope(var_t *var) {
    ...
    if (!root_scope_with) { //this is the first with_statement
        root_scope_with = (with_stmt_scope_t*)calloc(1,sizeof(with_stmt_scope_t));
        ...
        new_with_stmt_scope = (with_stmt_scope_t*)calloc(1,sizeof(with_stmt_scope_t));
